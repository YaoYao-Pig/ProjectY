...
import { emptyCatalog, validateCatalog, resolveEnum, fieldTypes } from './definitions.mjs';
...
// Lua 十进制转义按字节解释，确保 UTF-8 中文与控制字符无损写入。
...
export function encodeTable(table) {
  ...
  const annotations = table.fields.map(field => `---@field ${field.name} ${field.type === 'int' || field.type === 'float' ? 'number' : field.type === 'text' ? 'string' : field.type === 'formula' ? 'ConfigFormula' : field.type === 'enum' ? field.values.map(luaLiteral).join('|') : field.type === 'enum[]' ? '(' + field.values.map(luaLiteral).join('|') + ')[]' : field.type.replace('int', 'number').replace('float', 'number').replace('bool', 'boolean')}${field.description ? ' ' + field.description.replace(/[\r\n]/g, ' ') : ''}`).join('\n');
  return { binary: writer.finish(), lua: `-- 由 Tools/ConfigEditor/exporter.mjs 自动生成，请勿手动修改。\n---@class ${table.name}Row\n${annotations}\nreturn ${luaLiteral(schema)}\n` };
}
