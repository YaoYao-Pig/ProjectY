...
local ok,layout,views,lighting=xpcall(function()
    ...
    return areas:LayoutSnapshot(),result,require('Game.Rendering.MapPresentation').Snapshot(registry:Get('Config'))
end,debug.traceback)
registry:Shutdown();if not ok then error(layout,0) end
return layout,views,lighting
