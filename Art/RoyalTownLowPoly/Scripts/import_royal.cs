// 由 Unity MCP 定向导入王城资源，并同步真实 Demo 的资源引用；不进入 Play。
...
var report=Newtonsoft.Json.Linq.JArray.Parse(System.IO.File.ReadAllText(System.IO.Path.Combine(root,"Art/RoyalTownLowPoly/Integration/models.json")));
var palette=Newtonsoft.Json.Linq.JObject.Parse(System.IO.File.ReadAllText(System.IO.Path.Combine(root,"Art/RoyalTownLowPoly/Integration/palette.json")));
var target="Assets/DynamicAsset/RoyalTownLowPoly";
// 源表字段变化时同步导出的 TextAsset，避免 Editor 内仍缓存旧二进制结构。
foreach(var name in new[]{"MapAreaEntranceTable","MapAreaRoyalPlacementTable","MapAreaRoyalTable","MapAreaTable","MapAreaTownFacilityTable","MapAreaTownLotTable","MapAreaTownTable","MapAssetTable","MapTownTable"})
...
System.IO.File.WriteAllText(System.IO.Path.Combine(root,"Art/RoyalTownLowPoly/Integration/unity-import.json"),Newtonsoft.Json.JsonConvert.SerializeObject(results,Newtonsoft.Json.Formatting.Indented));
return results;
