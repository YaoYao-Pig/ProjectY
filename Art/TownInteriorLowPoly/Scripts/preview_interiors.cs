...
ProjectY.Samples.MapAreaViewData layout;var views=new System.Collections.Generic.List<ProjectY.Samples.AdventureViewData>();var names=new System.Collections.Generic.List<string>();
using(var lua=new XLua.LuaEnv()){
 ...
 try{
  var result=lua.DoString(System.IO.File.ReadAllText(System.IO.Path.Combine(root,"Art/TownInteriorLowPoly/Scripts/preview_interiors.lua")));
...
try{
 ...
 for(var n=0;n<views.Count;n++){
  var view=views[n];var state=view.Area;var name=names[n];renderer.UpdateVisibility(state,false);
  ...
  if(n==0){
   ...
   camera.orthographic=true;camera.orthographicSize=84;camera.transform.SetPositionAndRotation(center-rotation*UnityEngine.Vector3.forward*250,rotation);capture(name);
  }else{
   var walk=new ProjectY.Samples.TownWalkCamera(layout,state,renderer.CameraObstacles,(ray,limit)=>{int hit;return surface.Raycast(ray,limit,out hit);});walk.Apply(camera,squad.Position(state.Members[0].ActorId));capture(name+"-walk");
   ...
   if(interior>0 && name.EndsWith("interior")){
    ...
    var rotation=UnityEngine.Quaternion.Euler(58,prop.Rotation+170,0);camera.transform.SetPositionAndRotation(center-rotation*UnityEngine.Vector3.forward*80,rotation);capture(name+"-plan");
   }
  }
 }
 ...
 System.IO.File.WriteAllText(System.IO.Path.Combine(root,"Art/TownInteriorLowPoly/Integration/preview.json"),Newtonsoft.Json.JsonConvert.SerializeObject(report,Newtonsoft.Json.Formatting.Indented));return report;
}finally{if(renderer!=null)renderer.Dispose();if(surface!=null)surface.Dispose();if(npcs!=null)npcs.Dispose();if(squad!=null)squad.Dispose();UnityEditor.SceneManagement.EditorSceneManager.ClosePreviewScene(preview);}
