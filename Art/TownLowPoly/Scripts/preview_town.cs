...
ProjectY.Samples.SquadPawnRenderer squad=null;ProjectY.Samples.TownNpcRenderer npcs=null;ProjectY.Samples.TownSurfaceRenderer surface=null;
try{
 ...
 if(fields.FindProperty("assetBindings").arraySize!=53 || fields.FindProperty("pawnBindings").arraySize!=18)throw new System.Exception("Scene bindings not synchronized");
 ...
 camera.orthographic=true;camera.orthographicSize=60;var rotation=UnityEngine.Quaternion.Euler(53,155,0);camera.transform.SetPositionAndRotation(center-rotation*UnityEngine.Vector3.forward*300,rotation);capture("town-terraced-overview");
 var controller=new ProjectY.Samples.TownWalkCamera(layout,state,obstacles,(ray,limit)=>{int hit;return surface.Raycast(ray,limit,out hit);});controller.Apply(camera,squad.Position(state.Members[0].ActorId));capture("town-terraced-street");
 foreach(var current in new[]{bridgeView,underView}){
  ...
  capture(current==bridgeView?"town-terraced-bridge":"town-terraced-underpass");
 }
 var report=new{facilities=layout.Facilities.Length,npcs=layout.Npcs.Length,members=state.Members.Length,mapBindings=53,pawnBindings=18,checkedEdges=checkedEdges,playing=false,images=outputs};
 ...
 return report;
}finally{if(surface!=null)surface.Dispose();if(npcs!=null)npcs.Dispose();if(squad!=null)squad.Dispose();UnityEditor.SceneManagement.EditorSceneManager.ClosePreviewScene(preview);}
