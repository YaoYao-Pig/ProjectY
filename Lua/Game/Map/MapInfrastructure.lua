...
function Infrastructure:PlanTown(map, finder, random, row, center)
    local candidates, occupied, streets = {}, { [center] = true }, {}
    for _, cell in ipairs(map.regions[center.regionId].cells) do
        local span = distance(center, cell)
        if span >= 2 and span <= row.radius and not cell.waterLevel and not cell.townId then
        ...
        for _, candidate in ipairs(candidates) do
            ...
            if allowed then
                ...
                for _, entry in ipairs(entries) do
                    path = finder:FindPath(center, entry, function(a, b)
...
function Infrastructure:PlaceTowns(map, finder, random)
    for _, row in ipairs(self.rows) do
        local candidates, perRegion, attempts, count = {}, {}, {}, 0
        for _, cell in ipairs(map.cells) do
            if contains(row.regions, map.regions[cell.regionId].regionType) then
                local area = disk(map, cell, 1)
                local delta = area and relief(area)
                if delta and delta <= row.maxGroundDelta then
                    candidates[#candidates + 1] = { cell = cell, score = delta + random:Noise(cell.q, cell.r, 1, 7907 + row.id) * 0.7 }
                end
            end
        end
        table.sort(candidates, function(a, b) return a.score < b.score or (a.score == b.score and finder.indices[a.cell] < finder.indices[b.cell]) end)
        for _, candidate in ipairs(candidates) do
            if count >= row.maxCount then break end
            local center, allowed = candidate.cell, true
            local regionId = center.regionId
            if (perRegion[regionId] or 0) >= row.maxPerRegion or (attempts[regionId] or 0) >= row.siteAttempts then allowed = false end
            for _, town in ipairs(map.towns) do
                if distance(center, town.center) < math.max(row.minSpacing, town.minSpacing, row.radius + town.radius + 1) then allowed = false; break end
            end
            if allowed then
                attempts[regionId] = (attempts[regionId] or 0) + 1
                local plans = self:PlanTown(map, finder, random, row, center)
                if plans then
...
function Infrastructure:ConnectTowns(map, finder)
    local parent, pairs = {}, {}
    for _, town in ipairs(map.towns) do parent[town.id] = town.id end
    ...
    for a = 1, #map.towns do for b = a + 1, #map.towns do
        pairs[#pairs + 1] = { a = a, b = b, span = distance(map.towns[a].center, map.towns[b].center) }
    end end
...
function Infrastructure:Build(map)
    ...
    self:PlaceTowns(map, finder, Random(map.seed ~ 0x51f2a913))
