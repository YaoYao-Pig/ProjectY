...
- 常驻 System 继承 `Core.LuaSystem`，在 `Game.Systems` 注册依赖。注册器按依赖排序：全部 `OnInit` → 全部 `OnStart`；运行中派发 `Tick/FixedTick/LateTick/OnPause`，逆序 `OnShutdown`。当前顺序为 Config → Localization → PlayerModel → UI。
