...
- 目标格数默认读取 `Map.DefaultTargetCells`（10000），上限读取 `Map.MaxCells`（100000）；可填 1 万、2.5 万、5 万、10 万或自定义整数，留空回到一遍配方的小地图模式。请求和导出 JSON 包含 targetCells，服务继续调用真实 Lua 生成器；精确裁剪规则与数量上限见 [地图业务](../Business/Map.md)。生成期间显示已用秒数。
...
- V5 增加“选址方案”热力图和“汇水与河道”着色。灰格表示硬限制/最低分未通过；热力图记录样式开始选址时的分数，实际提交的中心指标与分数另存 town.siteMetrics/siteScore。普通聚落与偏远地点分开计数；地牢无道路网络。详情可查方案、净距、取水代价、累计汇水、目标扩宽半径与盆地溢出高度；“选址结果与未生成原因”记录候选/占地/间距/预算统计。指标定义和规则归 [地图业务](../Business/Map.md)，字段调参见 [V5 Review](../../Docs/MapGenerationV5-Review.md)。
...
- 导出 JSON 包含格子、邻接、Region、Border、连通水体、地貌权重、towns/buildings/roads/roadNetworks、assets/decorations/rivers/waterfalls、siteProfiles/siteDiagnostics、各格 siteScores、种子、配方、算法版本和源码/配表哈希；保存图片导出当前地图画布。索引从 1 开始，`regions[].buildings` 保留原有候选配置含义，顶层 `buildings` 才是生成的占地布局。复现历史算法仍需保留对应工程源码及配表版本，哈希不是源码存档。
...
- `previewShape=dungeon` 绘制独立石拱暗门与台阶；`cottage` 绘制低墙木顶矮屋，不改生成逻辑。审阅地牢与隐居群落可用 seed=20260921、配方 `1,2,3,4,5,6,1,4,5,6,1,5,2,6`，当前包含 3 个隐居群落与 4 处地牢；列表可定位，方案 501 可查看群落评分。
- 每次生成捕获 `Config/Tables`、Catalog 与项目 Lua 源文件，用**现有 exporter** 在临时目录导出，再通过**项目 xLua DLL** 执行 `ConfigSystem → MapSystem → MapGenerator`。所有 require 使用此次快照中的 Lua 与 _Gen schema，`ReadConfig` 读取此次导出的同格式二进制；源码/表变化下一次生成即生效，不使用过期的工程 _Gen 文件。
...
- 预览不改写工程源表或 Unity 导表产物；临时快照在成功或失败后清理，每个请求使用全新 Lua 状态。Python/xLua 宿主复用于 Lua 测试入口。接口仅监听 loopback，验证 Host/Origin 与会话 token；同时最多一个生成任务，运行超时 180 秒、输出上限 256 MiB，请求另限制最多 100000 格。错误展示 Lua 栈，保留并标明上次成功的画布。
- 工具读取磁盘上**已保存**的文件；配置网页尚未保存的草稿不参与生成。点击“刷新配置”更新选择器与参数摘要，点击“生成地图”始终读取最新磁盘数据。Canvas 按 32×32 轴坐标分块剔除视野外地块，包围范围包含水面、建筑与装饰高度。至少一万格且格子半径小于 4 像素时，用最大边 4096 像素的离屏几何缓存显示地形、水体、道路与城市地块，远景隐藏建筑/树木细节，放大自动恢复；缓存随显示参数改变而重建。格子数据与点击查询仍完整，视图帧率不代表游戏运行性能。
...
尺寸定向检查：`node --test Tools/MapPreview/tests/map-size.test.mjs` 默认验证一万格的精确数量、全图/区域连通和输入边界；PowerShell 设置 `$env:MAP_TEST_CELLS='100000'` 后可显式检查十万格，并输出耗时与 JSON 体积。大规模检查较慢，不加入常规小改动的默认验证范围。
...
- [renderer-culling.test.mjs](../../Tools/MapPreview/tests/renderer-culling.test.mjs)：局部视野剔除、全图保留及旋转/俯视角/高度放大下的可见面检查；`node --test Tools/MapPreview/tests/renderer-culling.test.mjs`。
